...
# Load dataset
df = pd.read_excel("Normalized_Hausa_Merged_verified.xlsx")  # Use your file path
...
def extract_date_from_question(question):
    patterns = [
        ...
        r"\b\d{2}\.\d{2}\.\d{4}\b",        # DD.MM.YYYY
	r"\b\d{1,2} (?:Muharram|Safar|Rabi’ul-Awwal|Rabi’ul-Thani|Jumada al-Awwal|Jumada al-Thani|Rajab|Sha’ban|Ramadan|Shawwal|Dhul-Qa’ada|Dhul-Hijja) \d	{4}\b",
	r"\b\d{1,2} ga(?:n)? watan (?:Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba)n? \d{4}\b",
	r"\b\d{1,2} ga watan (Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba) \d{4}\b",
        r"\b\d{1,2} (?:January|February|March|April|May|June|July|August|September|October|November|December) \d{4}\b",  # English Month
	"\b\d{1,2} ga(?: watan)? (?:Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba),? \d{4}\b",
        r"\b\d{1,2} ga (?:Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba),? \d{4}\b",  # Hausa with "ga"
        r"\b\d{1,2} (?:Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba) \d{4}\b",       # Hausa without "ga"
        r"\b\d{1,2} (?:Muharram|Safar|Rabi’ul-Awwal|Rabi’ul-Thani|Jumada al-Awwal|Jumada al-Thani|Rajab|Sha’ban|Ramadan|Shawwal|Dhul-Qa’ada|Dhul-Hijja) \d{4}\b",  # Hijri Hausa
        r"\b\d{1,2}\. (?:Januar|Februar|März|April|Mai|Juni|Juli|August|September|Oktober|November|Dezember) \d{4}\b",    # German DD.MM.YYYY
        r"^(?:Montag|Dienstag|Mittwoch|Donnerstag|Freitag|Samstag|Sonntag), der \d{1,2}\. (?:Januar|Februar|März|April|Mai|Juni|Juli|August|September|Oktober|November|Dezember) \d{4}$",  # German weekday
        r"\b\d{4}年\d{1,2}月\d{1,2}日\b",  # Chinese YYYY年MM月DD日
    ]
...
def baseline_tokenizer(date_str, format_type):
    ...
    try:
        ...
        elif format_type == 'YYYY年MM月DD日':
            return [date_obj.strftime('%Y'), '年', date_obj.strftime('%m'), '月', date_obj.strftime('%d'), '日']
        elif format_type == '%A, YYYY年MM月DD日':
            return [date_obj.strftime('%A'), ', ', date_obj.strftime('%Y'), '年', date_obj.strftime('%m'), '月', date_obj.strftime('%d'), '日']
        else:
            return [date_str]
    except:
        return [date_str]
...
def infer_date_format(date_str):
    format_guesses = [
        ('DD ga Month YYYY', r'^\d{1,2} ga (?:Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba)(?:n)?(?:,)? \d{4}$'),
        ('DD Month YYYY (Hausa)', r'^\d{1,2} (?:Janairu|Fabrairu|Maris|Afrilu|Mayu|Yuni|Yuli|Agusta|Satumba|Oktoba|Nuwamba|Disamba) \d{4}$'),
        ...
        ('YYYY.MM.DD', r'^\d{4}\.\d{2}\.\d{2}$'),
        ('YYYYMMDD', r'^\d{8}$'),
        ('YYYY年MM月DD日', r'^\d{4}年\d{1,2}月\d{1,2}日$'),
        ('%A, YYYY年MM月DD日', r'^(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday), \d{4}年\d{1,2}月\d{1,2}日$')
    ]
...
def label_period(year, date_str=None):  
    ...
    if is_hijri:
        ...
        if year < 2000:
            return "Historical (Pre-2000)", f"{century}{suffix} Century"
        elif 2000 <= year <= 2025:
